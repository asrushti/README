...
void item_master_populate(Item_master *user_master){
	...
	fp_grp = fopen("GroupMaster.txt", "r");
	fp_cat = fopen("CategoryMaster.txt", "r");
...
void item_master_update(GtkApplication *app, Window_grid *win_grid){
	...
	Window_grid *prev_state;
	
	/*A pointer which stores the current window information.*/
	prev_state = (Window_grid *)malloc(sizeof(Window_grid));
	prev_state = win_grid;
	...
	/*Again set the window parameters.*/
	win_grid = prev_state;
