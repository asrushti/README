...
void inward_bill_view(GtkWidget *widget, Window_grid *win_grid){
	...
	fp = fopen("data/InwardMaster.txt", "r");
	i = 0;
	j = 5;
...
/*long double get_amount(float rate, float qty, float vat){
...
void get_purchase_no(GtkWidget *widget){
	...
	long long int code, code1, purchase_no, supplier_ino, count = 1;
	char *group, *category, *pname, *name, *unit, vat_str[MAX_STR], amt_str[30], total_str[30], code_str[16], party_name[MAX_STR], *item_name, count_str[16];
	float rate, vat;
	double total, vat_amt, amt;
	
	fp = fopen("data/InwardMaster.txt", "r");
	
	while(fread(&purchase_no, sizeof(long long int), 1, fp)){
		g_print("Inside 1st loop\n");
		//printf("Purchase No : %lld\n", purchase_no);
		fread(&ch, sizeof(char), 1, fp);
		
		//fseek(fp, sizeof(long long int), SEEK_CUR);
		fread(&supplier_ino, sizeof(long long int), 1, fp);
		printf("Supplier Ino : %lld\n", supplier_ino);
		fread(&ch, sizeof(char), 1, fp);
	...
	/*Convert the long long int to string.*/
	sprintf(count_str, "%lld", count);
	gtk_label_set_text(GTK_LABEL(widget), count_str);
