...
gboolean check_escape(GtkWidget *widget, GdkEventKey *event, Window_grid *win_grid){
	if (event->keyval == GDK_KEY_Escape) {
		...
		if(win_grid->current_screen == MASTER_SUB_GRP){
			g_master_activate(win_grid->application, win_grid);
			return TRUE;
		}
		
		if(win_grid->current_screen == MASTER_SUB_CAT){
			c_master_activate(win_grid->application, win_grid);
			return TRUE;
		}
